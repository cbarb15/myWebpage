...
inner join User where User.Id = Faculty.UserId and User.FirstName=\"$firstName\" and User.LastName=\"$lastName\"");
    ...
    {
        $this->query = $this->database->query("insert into Role(Id, UserId, Role) VALUES(\"\", \"$this->userId\", \"DGS\")");
    ...
    {
        $arrayOfGPAs = array();
        $this->query = $this->database->query("select GPA from Student");
        while($object = $this->query->fetch(PDO::FETCH_OBJ))
        {
            array_push($arrayOfGPAs, $object);
        }
        return $arrayOfGPAs;
    }
}